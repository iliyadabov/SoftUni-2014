...
public class AngleUnitConverter {
	public static void main(String[] args) {
		...
		for (int i = 0; i < convertionsCount; i++) {
			...
			if (typeOfConvertionCharArray[0] == 'r')
				commands[lastCommandIndex + 1] = "r";
			else
				commands[lastCommandIndex + 1] = "d";
			lastCommandIndex += 2;
		}
		...
		for (int i = 0; i < convertionsCount; i++) {
			if (commands[lastCommandIndex + 1] == "r") {
				System.out.format("%.6f deg", (Math.toDegrees(Double
						.parseDouble(commands[lastCommandIndex]))));
				System.out.println();
			} else {
				System.out.format("%.6f rad", Math.toRadians(Double
						.parseDouble(commands[lastCommandIndex])));
				System.out.println();
			}
			lastCommandIndex += 2;
